...
DataBlob* CenterCrop(DataBlob *bottom, uint crop_h, uint crop_w){
    ...
    uint hstart = (uint)(floor((float)(bottom->h-crop_h)/2));
    uint wstart = (uint)(floor((float)(bottom->w-crop_w)/2));
    uint hend = hstart+crop_h;
    uint wend = wstart+crop_w;
    for (n=0;n<bottom->n;n=n+1){
        for (c=0;c<bottom->c;c=c+1){
            top_offset = n * top->c * top->h * top->w + c * top->h * top->w;
    ...
    //printf(">>>center crop: n:%d, c:%d, h:%d, w:%d\n",top->n, top->c, top->h, top->w);
    return top;
}
...
DataBlob* DataNormalize(DataBlob *bottom, const D_Type *mean, const D_Type *std_dev){
    ...
    //printf(">>>data normalize: n:%d, c:%d, h:%d, w:%d\n",top->n, top->c, top->h, top->w);
    return top;
}
